...
region=NetCDFFile('/scratch2/scratchdirs/tslin2/isam/maisoy_cheyenne/code/isamhiscru_mai_luh2rcp85new.nc','r')
ma1 = region.variables['yield'][75:85,:,:]
#ma1 = region.variables['yield'][99,:,:]

region1=NetCDFFile('/scratch2/scratchdirs/tslin2/isam/maisoy_cheyenne/code/isamhiscru_soy_luh2rcp85new.nc','r')
ma2 = region1.variables['yield'][75:85,:,:]
#ma2 = region1.variables['yield'][99,:,:]

ma1=N.ma.average(ma1,axis=0)
ma2=N.ma.average(ma2,axis=0)
ma1h=N.ma.average(ma1h,axis=0)
ma2h=N.ma.average(ma2h,axis=0)
...
map.drawmapboundary()
#ncvar_y=maskoceans(x1,y1,ncvar_y)
ab=(ma2-ma2h)/ma2h*100
ab= ma.masked_where(ab==0.0,ab)

cs1 = map.pcolormesh(x,y,ab,cmap=cmap,norm=norm)
#cs1 = map.pcolormesh(x,y,ma2,cmap=plt.cm.gist_earth,vmin=0,vmax=5)
plt.axis('off')
cbar = map.colorbar(cs1,location='bottom',size="5%",pad="2%",ticks=bounds,extend='both')
cbar.ax.tick_params(labelsize=16)

plt.savefig('isam_maisoy2090srcp85diffnew.jpg',bbox_inches='tight')
