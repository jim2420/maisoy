...
eiyield5ynew = edat5.variables['totalyield'][0:115,:,:]
 



dat=NetCDFFile('/scratch2/scratchdirs/tslin2/isam/maisoy_cheyenne/his_cru/fixedirr/mai_irr_fert/output/mai_irr_fert.nc','r')
iyield1ynew = dat.variables['totalyield'][0:115,:,:]
latisam=dat.variables['lat'][:]
lonisam=dat.variables['lon'][:]
dat2=NetCDFFile('/scratch2/scratchdirs/tslin2/isam/maisoy_cheyenne/his_cru/fixedirr/mai_co2_irr/output/mai_co2_irr.nc','r')
iyield2ynew = dat2.variables['totalyield'][0:115,:,:]

dat2=NetCDFFile('/scratch2/scratchdirs/tslin2/isam/maisoy_cheyenne/his_cru/mai_fert/output/mai_fert.nc','r')
aiyield2ynew = dat2.variables['totalyield'][0:115,:,:]


dat3=NetCDFFile('/scratch2/scratchdirs/tslin2/isam/maisoy_cheyenne/his_cru/fixedirr/mai_co2_fert/output/mai_co2_fert.nc','r')
iyield3ynew = dat3.variables['totalyield'][0:115,:,:]

dat2=NetCDFFile('/scratch2/scratchdirs/tslin2/isam/maisoy_cheyenne/his_cru/fixedirr/mai_irr/output/mai_irr.nc','r')
aiyield3ynew = dat2.variables['totalyield'][0:115,:,:]
...
dat5=NetCDFFile('/scratch2/scratchdirs/tslin2/isam/maisoy_cheyenne/his_cru/fixedirr/mai_co2_cli/output/mai_co2_cli.nc','r')
iyield5ynew = dat5.variables['totalyield'][0:115,:,:]
dat2=NetCDFFile('/scratch2/scratchdirs/tslin2/isam/maisoy_cheyenne/his_cru/fixedirr/mai_cli/output/mai_cli.nc','r')
aiyield5ynew = dat2.variables['totalyield'][0:115,:,:]
...
iyield6ynew = dat6.variables['totalyield'][0:115,:,:]
...
aiyield2ynew= ma.masked_where(aiyield2ynew<=0.,aiyield2ynew)
aiyield3ynew= ma.masked_where(aiyield3ynew<=0.,aiyield3ynew)
aiyield5ynew= ma.masked_where(aiyield5ynew<=0.,aiyield5ynew)
...
esco=N.zeros((9))
escli=N.zeros((9))
areasum=N.sum(maizeto2,axis=(1,2))
...
ballyinew=N.sum(aiyield2ynew*maizeto2,axis=(1,2))
ballynnew=N.sum(aiyield3ynew*maizeto2,axis=(1,2))
ballyclinew=N.sum(aiyield5ynew*maizeto2,axis=(1,2))
...
for i in range (0,6):
	...
	if i<=4:
		c2=c1+10
	        ...
	        c2=c1+6
	av1[i+1]=N.average(allynew[c1:c2])/(10**6)
        av2[i+1]=N.average(allyinew[c1:c2])/(10**6)
        av3[i+1]=N.average(allynnew[c1:c2])/(10**6)
        av4[i+1]=N.average(allycnew[c1:c2])/(10**6)
        av5[i+1]=N.average(allyclinew[c1:c2])/(10**6)

        bav2[i+1]=N.average(ballyinew[c1:c2])/(10**6)
        bav3[i+1]=N.average(ballynnew[c1:c2])/(10**6)
        bav5[i+1]=N.average(ballyclinew[c1:c2])/(10**6)

	aarea[i+1]=N.average(areasum[c1:c2])/(10**6)
...
ax.plot(gx,bav2,"ro-",label="S_Irri",linewidth=2)
ax.plot(gx,av2,"r*--",label="S_CO$_{2}$_Irri",linewidth=2)

ax.plot(gx,bav3,"go-",label="S_N",linewidth=2)
ax.plot(gx,av3,"g*--",label="S_CO$_{2}$_N",linewidth=2)

ax.plot(gx,bav5,"bo-",label="S_Cli",linewidth=2)
ax.plot(gx,av5,"b*--",label="S_CO$_{2}$_Cli",linewidth=2)
...
print (av1[6]-av4[6])/aarea[6],(bav2[6]-av2[6])/aarea[6],(bav3[6]-av3[6])/aarea[6],(bav5[6]-av5[6])/aarea[6]

leg = plt.legend(loc=2,ncol=4, fontsize=16)
leg.get_frame().set_alpha(0.5)
#plt.xlabel("Year",fontsize=18)
plt.ylabel("Production (Million tons)",fontsize=18)

plt.tick_params(axis='both',labelsize=18)
#ax.xaxis.set_major_formatter(DateFormatter('%Y'))
plt.savefig('maize_yearfixed.png',dpi=300,bbox_inches='tight')
plt.show()
