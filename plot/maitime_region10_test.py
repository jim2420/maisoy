...
#                ax.plot(gx,locals()['e{0}p451'.format(xy)][8,5:20],"g-",label="SSEA",linewidth=1)
                ax.plot(gx,locals()['e{0}p451'.format(xy)][7,5:20],"k-",label="SSP5RCP8.5",linewidth=2)
                ax.plot(gx,locals()['e{0}p451'.format(xy)][8,5:20],"k--",label="SSP2RCP4.5",linewidth=2)
...
leg = plt.legend(loc=2,ncol=7, fontsize=14)
...
plt.savefig('maize_regiontime10_test1.png',dpi=300,bbox_inches='tight')
plt.show()
