...
for i in xrange(1):
	ax = fig.add_subplot(5,2,i+1)
        ...
        ax.text(0.05,.03,'{0}'.format(name[i]),fontsize=14,
...
#	ax.set_title('{0}'.format(name[i]),fontsize=12)
	...
	ax.set_xticks([0.015,0.025,0.035,0.045])
#        if idx==0 or idx==3 or idx==6:
        ax.set_xticklabels(["CO$_{2}$","Climate","Irrig","NF"])
	...
	for j in range(0,1):
		...
		if j==0:
			...
			xx=x1+1
                print ['{0}p'.format(xy)]
                plt.bar(0.01+index+bar_width*x1,locals()['{0}p451'.format(xy)][i,19],bar_width,hatch='x',facecolor='white',alpha=opacity,edgecolor=['{0}'.format(cc)])
...
#                plt.bar(0.01+index+bar_width*xx,locals()['{0}p851'.format(xy)][i,19],bar_width,alpha=opacity,color=['{0}'.format(cc)])
                gc=gc+1
plt.legend(['SSP2RCP4.5','SSP5RCP8.5'])
plt.savefig('maize_regiontime10barfuturetest.png',dpi=300,bbox_inches='tight')
plt.show()
