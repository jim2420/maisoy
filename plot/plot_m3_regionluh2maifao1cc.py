...
#fao 1997-200
afy1=[4.1511,4.4354,4.4255,4.3236,4.4775,4.3884,4.4610,4.9452,4.8196];afp1=[584401847,615072804,607426254,592030667,615143531,603544019,645048171,729511789,714185792]
afy2=[7.9149,8.4224,8.3814,8.5072,8.5838,8.0696,8.8794,10.0004,9.2617];afp2=[241044300,256834420,248709880,258807600,249765850,236765680,265816320,308712370,291594750]
afy3=[2.662938072,2.902123718,2.799827206,2.850427769,3.159183373,3.094081952,3.472247437,3.312799625,3.37964777]
afp3=[75091967,75963869,72428772,76623680,88149351,80649743,96239369,90948617,87951548]
afy4=[6.7681,5.9292,6.5432,5.5854,6.4164,6.5012,5.3675,7.1082,7.0232];afp4=[64463531,55704666,61471174,52759921,63281368,62357088,54024458,73848833,65392054]
afy5=[1.6209,1.5910,1.7458,1.8062,1.7287,1.7531,1.6129,1.7389,1.7031];afp5=[41513535,39895544,42484053,43798248,41440337,44959371,45596239,48287165,50366111]

afy7=[3.1366,1.5951,1.9055,2.0660,1.7299,2.7422,3.0813,3.8752,3.8500];afp7=[2652012,799940,1033569,1489399,808360,1499425,2030896,3372956,3060164]
afy8=[4.3901,5.2686,4.9458,4.5992,4.6999,4.8737,4.8146,5.1217,5.2881];afp8=[104647617,133197612,128287195,106178315,114255995,120188915,115997909,130434297,139498473]
afy9=[2.040386526,2.148397666,2.147994643,2.216194937,2.327170041,2.271333577,2.484977744,2.559675565,2.629582297]
afp9=[34109164,36360277,36499277,37878785,39174375,38026401,44690707,47046627,49686749]

N.savetxt('maimeaurefao.csv', (afy1,afy2,afy3,afy4,afy5,afy8,afy9), delimiter=',')
N.savetxt('maimeaurem3.csv', (allym3[0],allym3[1],allym3[2],allym3[3],allym3[4],allym3[7],allym3[8]), delimiter=',')
N.savetxt('maimeaurespam.csv', (spamyy[0],spamyy[1],spamyy[2],spamyy[3],spamyy[4],spamyy[7],spamyy[8]), delimiter=',')
N.savetxt('maimeaureilly.csv', (iially[0,:],iially[1,:],iially[2,:],iially[3,:],iially[4,:],iially[7,:],iially[8,:]), delimiter=',')
...
N.savetxt('maimeaure.csv', (allyisam1,faoy1,faoy1_std), delimiter=',')
...
leg=plt.legend(loc=1,ncol=1, fontsize=50)
#leg.get_frame().set_alpha(0.5)
plt.tick_params(axis='both',labelsize=50)


plt.tight_layout()
plt.savefig('mai_regionyf1.png')
plt.show()
...
rects1 = plt.bar(index+0.1, allproisam1, bar_width,yerr=allproisam1_std,ecolor='black', capsize=10,
                 alpha=opacity,
                 color='blue',
                 label='ISAM')
rects0 = plt.bar(index + 0.1+bar_width*1, faop1, bar_width,yerr=faop1_std,ecolor='black', capsize=10,
                 alpha=opacity,
                 color='r',
                 label='Other studies')
...
bar_width = 0.4
opacity = 1.0
rects1 = plt.bar(index+0.1, isamgrid1, bar_width,yerr=isamgrid1_std,ecolor='black', capsize=10,
                 alpha=opacity,
                 color='blue',
                 label='ISAM')
...
#                 color='g',
#                 label='M3')
rects0 = plt.bar(index + 0.1+bar_width*1, faoyy1, bar_width,yerr=faoyy1_std,ecolor='black', capsize=10,
                 alpha=opacity,
                 color='r',
                 label='Other studies')
...
plt.tight_layout()
plt.savefig('mai_regiony1f1.png')
plt.show()
