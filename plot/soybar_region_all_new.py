...
#maitrop = region1.variables['area'][60,:,:]
lonisam1=region1.variables['lon'][:]
maitrop=ma.masked_where(maitrop<=0,maitrop)
maitrop=ma.filled(maitrop, fill_value=0.)
#maizeto=N.zeros((10,360,720))
...
for i in range(0,10):
	maizeto1[i,:,:],lonisam2=shiftgrid(0.5,maizeto[i,:,:],lonisam1,start=True)
	maizeto1r[i,:,:],lonisam2=shiftgrid(0.5,maitrop[i,:,:],lonisam1,start=True)
...
#        eallyclinew=N.sum(eiyield5ynew*maizeto3,axis=(1,2))
        ...
        '''
        ebb=(eallyinew-eallynew)
        ecc=(eallynnew-eallynew)
        edd=(eallycnew-eallynew)
        eee=(eallyclinew-eallynew)
	'''
        eii[i]=N.average(ebb)
        enn[i]=N.average(ecc)
        eco[i]=N.average(edd)
...
print eco[8],ecli[8],eii[8],enn[8]
print co[8],cli[8],ii[8],nn[8]
...
g=4
N.savetxt('soyhis.csv', (eco[g],co[g],ecli[g],cli[g],eii[g],ii[g],enn[g],nn[g]), delimiter=',')

plt.savefig('soy_region_id_new.png')
plt.show()
