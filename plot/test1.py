...
#        if i==6 or i==7:
	...
	print ['e{0}p'.format(xy)]
        plt.bar(0.01+index+bar_width*1,locals()['e{0}p451'.format(xy)][0,9],bar_width,alpha=opacity,color='b',label='Non-Interactive')
	...
	xy=gge[gc]
        print ['{0}p'.format(xy)]
        plt.bar(0.01+index+bar_width*2,locals()['{0}p451'.format(xy)][0,9],bar_width,alpha=opacity,color='r',label='Interactive')
...
leg = plt.legend(loc=1,ncol=2, fontsize=16)
...
plt.savefig('soy_regiontime10barluyield1ptest.png',dpi=300,bbox_inches='tight')
plt.show()
